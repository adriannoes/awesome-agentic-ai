...
## Security note on the browser launcher

`scripts/server.cjs` opens the user's browser when the companion is accepted with `--open`. A `skill-security-auditor` scan flags this as CRITICAL because it uses Node's `child_process`. The usage is intentional and safe by construction:

- **Opt-in only** — `maybeOpenBrowser()` returns immediately unless `BRAINSTORM_OPEN` is set, and only on `127.0.0.1`/`localhost`.
- **Per-session key gates every request** — the companion URL carries a one-time key; requests without it are 403'd, so a stray tab or remote host cannot drive the session.
- **Default launcher uses `execFile`, not a shell** — the platform launcher (`open`/`xdg-open`) receives the URL as an argv element, so shell metacharacters in a URL cannot inject a command.
- **`BRAINSTORM_OPEN_CMD` is trusted operator input** — when set, it runs as given. This is an environment variable the operator controls locally, not untrusted agent/user data, and it is documented inline in `server.cjs` as such.

No change is warranted; the flag documents why `child_process` is present rather than removing it.
