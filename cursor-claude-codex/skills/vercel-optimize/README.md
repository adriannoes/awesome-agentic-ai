...
A `skill-security-auditor` scan flags this skill with many CRITICAL findings because `lib/` and `scripts/` use Node's `child_process` and `RegExp.prototype.exec`. Both are safe by construction here:

- **Shell-outs use `execFile`, not `exec`.** `lib/vercel.mjs` documents this inline: "All shell-outs use execFile (not exec) — no shell injection." Arguments are passed as an array, so no shell metacharacter injection surface exists. The `exec` symbol in that file is `promisify(execFile)`.
- **`regex.exec(text)` is `RegExp.prototype.exec`, not `child_process.exec`.** The auditor pattern-matches the bare token `exec` and cannot distinguish the two. The matches in `workspace-resolver.mjs`, `sanitizers/undeclared-dep.mjs`, and similar files are regex iteration, not command execution.
- **No untrusted input reaches the shell.** Inputs are the user's own Vercel project files and CLI output parsed as JSON first; stderr is grepped only as a fallback.

No change is warranted; this note documents why `child_process` and `.exec()` are present.
